...
def train_misa(args, model, train_loader, valid_loader, test_loader):
    ...
    def get_sim_loss():
        sim = CMD()
        loss = sim(model.shared_h_l, model.shared_h_a, 5)
        loss += sim(model.shared_h_l, model.shared_h_v, 5)
        loss += sim(model.shared_h_v, model.shared_h_a, 5)
        return loss / 3.0

    def get_dif_loss():
        dif = DiffLoss()
        loss = dif(model.shared_h_l, model.private_h_l)
        loss += dif(model.shared_h_a, model.private_h_a)
        loss += dif(model.shared_h_v, model.private_h_v)
        loss += dif(model.private_h_l, model.private_h_v)
        loss += dif(model.private_h_l, model.private_h_a)
        loss += dif(model.private_h_v, model.private_h_a)
        return loss

    def get_rec_loss():
        rec = MSE()
        loss = rec(model.h_l, model.recon_h_l)
        loss += rec(model.h_a, model.recon_h_a)
        loss += rec(model.h_v, model.recon_h_v)
        return loss / 3.0
    ...
    def train():
        ...
        for i_batch, (batch_X, batch_Y, batch_META) in enumerate(train_loader):
            ...
            loss = criterion(pred, eval_attr)
            loss += args.sim_weight * get_sim_loss()
            loss += args.dif_weight * get_dif_loss()
            loss += args.rec_weight * get_rec_loss()
